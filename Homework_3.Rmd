...
Oldest_Lived <- titanic_train %>%
  select(Survived, Age) %>%
  mutate(Survived = str_replace(Survived, "0", "Dead"),
         Survived = str_replace(Survived, "1", "Survived")) %>% 
  mutate(Lived = (str_extract(Survived, "Survived"))) %>% 
  mutate(Died = (str_extract(Survived, "Dead"))) %>% 
  select(Age, Lived, Died) %>% 
  arrange(desc(Age)) %>% 
  arrange(Lived) %>% 
  select(Age, Lived) %>% 
  slice(1:5)
Oldest_Died <- titanic_train %>%
  select(Survived, Age) %>%
  mutate(Survived = str_replace(Survived, "0", "Dead"),
         Survived = str_replace(Survived, "1", "Survived")) %>% 
  mutate(Lived = (str_extract(Survived, "Survived"))) %>% 
...
Oldest_dead_live <- full_join(Oldest_Lived, Oldest_Died)  
Oldest_dead_live
```
Histogram
```{r message=FALSE, warning=FALSE, error=FALSE}
