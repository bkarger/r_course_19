...
m_eight_letters <- whole_words %>% 
  as_tibble() %>% 
  rename(words = `2`) %>% 
  mutate(words = str_to_lower(words)) %>% 
  filter(words == str_extract_all(words, pattern = "[\\<m].[^m]{7}")) %>%
  mutate(first_word = stri_sub(words, from = 1, to = 4),
         second_word = stri_sub(words, from = 5, to = 8),
         first_letter = stri_sub(second_word, from = 1, to = 1),
         restofword = stri_sub(second_word, from = 2, to = 4),
         pasted_2ndword = paste(restofword, first_letter, sep = ""),
         pasted_whole2ndword = paste(first_word, pasted_2ndword, sep ="")) %>% 
  rename(original = words) %>% 
  distinct() 

m_eight_letters %>% 
  mutate(trying_matching = str_detect(m_eight_letters, pasted_2ndword)) %>%
  group_by(trying_matching) %>% 
  filter(trying_matching == TRUE) 
